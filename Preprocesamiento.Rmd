...
residuales_out_arima_total_4 <- (abs(residuales_arima_total_4)>2000)*1
...
A <- outliersdiag
outliersdiagdummy <- A[,A[row(A)==col(A)]==1]
...
auto.arima(ts_total,ic = "aic", xreg = outliersdiagdummy)

arima_total_4_dummy <- sarima(ts_total,2,1,1,2,0,0, S = 12, xreg = outliersdiagdummy , details = FALSE)

arima_total_4_dummystats <- stats::arima(x = ts_total, order = c(1, 1, 1), seasonal = list(order = c(2, 0, 0), period = 12), xreg = outliersdiagdummy , include.mean = TRUE, optim.control = list(trace = 1))

#arima_total_4_dummystats <- stats::arima(x = ts_total, order = c(1, 1, 1), seasonal = list(order = c(2, 0, 0), period = 12) , include.mean = TRUE, optim.control = list(trace = 1))

pry_dummy = matrix(1,36,dim(outliersdiagdummy)[2])

forecast_dummy_total <- predict(arima_total_4_dummystats,n.ahead = 36, newxreg = pry_dummy)

#forecast_dummy_total <- predict(arima_total_4_dummystats, n.ahead = 36)
```
...
# ggplot
ggplot() + geom_ribbon(data = forecast_frame, aes(x = date, ymin = Proyección - 2*Error, ymax = Proyección+2*Error, fill = "2 sd"  ), fill = "grey90")  + geom_ribbon(data = forecast_frame, aes(x = date, ymin = Proyección -Error, ymax = Proyección+Error,fill = "1 sd"  ), fill = "grey80") + geom_line(data = forecast_frame, aes(x = date, y = Proyección, colour = "Proyección")) + geom_line(data = hist_frame, aes(x = date, y = Historico, colour = "Histórico")) + xlab("Población carcelaria sindicada") + theme_minimal() + theme(legend.title=element_blank()) -> graf_pry_total_3
graf_pry_total_3
...
# ggplot
ggplot() + geom_ribbon(data = forecast_frame, aes(x = date, ymin = Proyección - 2*Error, ymax = Proyección+2*Error, fill = "2 sd"  ), fill = "grey90")  + geom_ribbon(data = forecast_frame, aes(x = date, ymin = Proyección -Error, ymax = Proyección+Error,fill = "1 sd"  ), fill = "grey80") + geom_line(data = forecast_frame, aes(x = date, y = Proyección, colour = "Proyección")) + geom_line(data = hist_frame, aes(x = date, y = Historico, colour = "Histórico")) + xlab("Población carcelaria condenada") + theme_minimal()+ theme(legend.title=element_blank()) -> graf_pry_conde_1
graf_pry_conde_1
