...
model.hurtos1$par
plot.states <- plot(model.hurtos1, plot.type = "states")
...
```{r salvar}
pry_df %>% group_by(Periodo) %>% mutate(total = Sindicados + Condenados)%>%  summarise(pry_ppl_ss = sum(total)) -> pry_ppl_total_ss

write.csv(pry_ppl_total_ss,"pry_ppl_total_ss.csv",row.names=FALSE)
```
