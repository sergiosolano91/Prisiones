...
model.hurtos <- MARSS(hurto, model = model.list, control = list(safe=TRUE, maxit=1000))
model.hurtos$par
#plot(model.hurtos)
```
...
estructuras %>% filter(Género == "Mujeres") %>%  filter(!(Delito == "NA")) %>%  ggplot() + geom_line(aes(x=fecha,y=Valor,colour = Estado)) + facet_wrap(~str_wrap(Delito,25), ncol = 4, scales = "free_y") + theme(legend.position = "top") + labs(colour = " ", y = "Población privada de la libertad - Mujeres") + ylim(c(0,NA)) + theme_minimal() -> delitosmujeres
delitosmujeres
```

```{r delitoshombres, fig.width=8, fig.height=11}
estructuras %>% filter(Género == "Hombres") %>%  filter(!(Delito == "NA")) %>%  ggplot() + geom_line(aes(x=fecha,y=Valor,colour = Estado)) + facet_wrap(~str_wrap(Delito,25), ncol = 4, scales = "free_y") + theme(legend.position = "top") + labs(colour = " ", y = "Población privada de la libertad - Hombres") + ylim(c(0,NA)) + theme_minimal()-> delitoshombres
delitoshombres
```


```{r delito, fig.width=6, fig.height=6}
estructuras %>% group_by(Delito, Género) %>% summarise(Media = mean(Valor)) %>% arrange(- Media)-> estructuras_resumen 

estructuras_resumen   %>%  ggplot()+geom_bar(aes(x=reorder(str_wrap(Delito,40),Media),y=if_else(Género=="Hombres",-Media,Media), fill = Género),stat = "identity") + coord_flip() + labs(x = "Delito", y = "Población promedio") + scale_y_continuous(labels = abs) + theme_minimal()+ scale_fill_manual(values = wes_palette("Moonrise3")) ->delitoperfil
delitoperfil
```
